...
export default function VisitorViewerPage() {
  ...
  useEffect(() => {
    ...
    const fetchOverview = async () => {
      try {
        const { data } = await api.get(`/s/${slug}/view?type=overview`, {
  ...
  const handleFileClick = async (node: RepoNode) => {
    try {
      ...
      const { data } = await api.get(`/s/${slug}/view?type=file&path=${encodeURIComponent(node.path)}`);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-silver-50">
        ...
        <p className="text-silver-500 font-bold uppercase tracking-widest text-xs">Loading Secure Content...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <header className="h-20 glass-bg border-b border-silver-200/50 flex items-center justify-between px-6 shrink-0 z-10">
        <div className="flex items-center gap-4">
          <div className="h-10 w-10 bg-modernGray-900 rounded-xl flex items-center justify-center shadow-md">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className="px-2.5 py-1 rounded-lg bg-green-50 text-green-600 text-[10px] font-bold uppercase tracking-widest border border-green-100 flex items-center gap-1.5 shadow-sm">
                <ShieldCheck className="h-3.5 w-3.5" /> Secure Access
              </span>
            </div>
            <p className="text-[10px] text-silver-400 font-bold uppercase tracking-widest mt-1.5">Authorized Visitor View</p>
          </div>
        </div>
        <div className="hidden sm:flex items-center gap-2 text-silver-400 font-bold text-[10px] uppercase tracking-widest bg-silver-50 px-3 py-1.5 rounded-lg border border-silver-100">
