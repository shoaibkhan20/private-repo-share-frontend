...
export default function VisitorViewerPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-zinc-950 relative overflow-hidden bg-grid-pattern">
        ...
        <p className="text-zinc-400 font-medium tracking-wide">Loading secure viewer...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-zinc-950 text-zinc-100 relative overflow-hidden bg-grid-pattern">
      ...
      {/* Header */}
      <header className="h-14 border-b border-zinc-855 flex items-center justify-between px-4 sm:px-6 bg-zinc-950/80 backdrop-blur-md shrink-0 relative z-10">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 text-zinc-100 font-bold text-sm">
            <Github className="h-4 w-4 text-zinc-400" />
            {repoInfo?.name || 'Repository'}
          </div>
          <span className="px-2.5 py-0.5 rounded-full bg-emerald-950/50 border border-emerald-900/40 text-emerald-400 text-[10px] font-semibold flex items-center gap-1">
            <ShieldCheck className="h-3 w-3" /> Secure View
          </span>
        </div>
      </header>
